...
class YMODEM(object):
    ...
    def __init__(self, getc, putc, mode='ymodem', header_pad=b'\x00', pad=b'\x1a'):
        self.getc = getc #serial read
        self.putc = putc #serial write
        self.mode = mode #modem type
    ...
    def abort(self, count=2):
        for _ in range(count):
            self.putc(CAN)          #cancel signal
         
    '''
    send entry
    '''
    def send(self, file_stream, file_name, file_size=0, retry=20, timeout=30, callback=None):
        try:
            packet_size = dict(
                ymodem10k = 10240,                
                ymodem = 1024,
        ...
        while True:
            ...
            if char:
                if char == CRC:
                    # Expected CRC
                    self.log.info("<<< CRC")        #C from MCU
                    break
                elif char == CAN:
                    self.log.info("<<< CAN")        #from MCU
                    if cancel:
                        return False
                    else:
                        cancel = 1
            ...
            if error_count > retry:
                self.abort()            #send abort
                self.log.error("send error: error_count reached %d aborting", retry)
                return False

        header = self._make_send_header(USER_SIZE, 0)         #128 original
        ...
        data = data.ljust(USER_SIZE, self.header_pad)         #128 original
        
        checksum = self._make_send_checksum(data)
        data_for_send = header + data + checksum
        self.putc(data_for_send)            #header head
        ...
        header = self._make_send_header(USER_SIZE, 0)         #original 128
        
        data = bytearray(b'\x00')
 
        data = data.ljust(USER_SIZE, self.header_pad)             #original 128
    ...
    def _make_send_header(self, packet_size, sequence):
        assert packet_size in (128, 1024,10240), packet_size
        _bytes = []
        if packet_size == 10240:
            _bytes.append(ord(SZH))
